...
p "ORIG" + original_stops.to_s
p original_stops.index("Linlithgow")

# 5. Remove `"Livingston"` from the array using its name
stops.delete("Linlithgow")
p stops
# 6. Delete `"Cumbernauld"` from the array by index
ind = stops.index("Cumbernauld")
p ind
stops.delete_at(ind)
p "Removed Cumbernauld #{stops}"
# 7. How many stops there are in the array?
p "Number of stops are: #{original_stops.count}"
...
p "Erik's updated lott nos: #{users ["Erik"][:lottery_numbers]}"
...
users ["Rambo"] = {
  :twitter => "BalboaR",
  :lottery_numbers => [12, 45, 31, 42, 21, 46],
  :home_town => "Hollywood",
  :pets => [
    {
      :name => "balboa",
